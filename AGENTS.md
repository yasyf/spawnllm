...
├── scripts/          # build_wasm.sh — builds the blob into go/internal/core/ and spawnllm/_core/
...
All drift-prone logic — argv planning, envelope resolution, strict-schema transforms, JSON extraction, retry policy, auth probes, isolation seeding — lives once in `rust/spawnllm-core`. Python and Go embed it as a wasm blob; the Rust crate links it natively. The blob is never committed: `task blob` (or `bash scripts/build_wasm.sh`) builds it into both host trees, and it needs `rustup target add wasm32-wasip1` once. A missing blob fails loudly (Go: compile error; Python: `FileNotFoundError` naming the script).
