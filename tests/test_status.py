...
from subllm import ClaudeNotAuthenticated, ClaudeNotInstalled, ClaudeReady, check_status
...
class TestCheckStatus:
    def test_not_installed_with_brew(self) -> None:
        ...
        with patch("subllm.backends.claude.shutil.which", side_effect=which):
            assert check_status() == ClaudeNotInstalled(brew_available=True)

    def test_not_installed_without_brew(self) -> None:
        with patch("subllm.backends.claude.shutil.which", return_value=None):
    ...
    def test_ready_when_auth_status_zero(self) -> None:
        ...
        with (
            patch("subllm.backends.claude.shutil.which", return_value="/usr/bin/claude"),
            patch("subllm.backends.claude.subprocess.run", return_value=done),
        ):
